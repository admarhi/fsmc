...
sol_to_exchange_network = function(sol_tmp,met_medium,filterLow=1e-03){
  ...
  net.names=t(sapply(sol.names,FUN=function(x){stri_split(x,fixed="_e_")[[1]]}))
  net.names=as.data.frame(net.names)
  colnames(net.names)=c("nodeA","nodeB")
  net.names$nodeA=stri_replace_first(net.names$nodeA,fixed="R_EX_",replacement="")
  net.names$nodeB=stri_replace_first(net.names$nodeB,fixed="_i",replacement="")
  net.df=net.names
